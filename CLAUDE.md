...
- [~] **Phase 3 — Next.js 網站**:✅ 起好 + ✅ **已上線 Vercel**(https://flight-deal-system.vercel.app,2026-06-16;Supabase 數據正常,root dir=`web`,**production branch=`main`,push main 即自動 deploy**)。PR #6 + #7 已把全部網站 work merge 入 main(53 個目的地 / 仍在開發中 / 合併比價卡 / 過時數據自動清 已全部 live)。Filter row 對齊修正(label 同 chip 分兩欄 → 7–12日 換行 chip + 預算輸入框全部左對齊)喺 `phase-3-website`,**待 PR #8 merge 落 main 即生效**。喺 branch `phase-3-website`。詳見下面「網站現狀」。
